...
class HrPayslip(models.Model):
    """
        Represents the employee's salary slip for a given period.

        Purpose:
        - Generates salary computation based on contracts, rules, inputs, and worked days.
        - Manages states of payslips (Draft, Waiting, Done, Rejected).
        - Handles refund payslips and salary sheet computations.
        - Provides integration with email notifications and reporting.
    """
    _name = 'hr.payslip'
    ...
    def _compute_details_by_salary_rule_category(self):
        """Compute payslip lines grouped by salary rule categories."""
        for payslip in self:
            payslip.details_by_salary_rule_category =\
                payslip.mapped('line_ids').filtered(lambda line: line.category_id)

    def _compute_payslip_count(self):
        """Compute the number of lines in the payslip."""
        for payslip in self:
    ...
    def action_payslip_draft(self):
        """Set payslip status back to draft."""
        return self.write({'state': 'draft'})

    def action_payslip_done(self):
        """Validate the payslip and compute sheet."""
        self.compute_sheet()
    ...
    def refund_sheet(self):
        """
            Generate a refund payslip by duplicating current payslip
            and marking it as a credit note.
        """
        for payslip in self:
        ...
        return {
            ...
            'views': [
                (tree_view_ref and tree_view_ref.id or False, 'list'),
    ...
    def action_send_email(self):
        """Send payslip to employee via email template."""
        self.ensure_one()
        ir_model_data = self.env['ir.model.data']
        try:
            template_id = self.env.ref('jm_hr_payroll.mail_template_hr_payslip_employee').id
        except ValueError:
            template_id = False
        try:
    ...
    def unlink(self):
        """Restrict deletion of non-draft or non-cancelled payslips."""
    ...
    def get_contract(self, employee, date_from, date_to):
        """
            ...
            @return: returns the ids of all the contracts for the given
            employee that need to be considered for the given dates
        """
    ...
    def compute_sheet(self):
        for payslip in self:
            ...
            if not contract_ids:
                raise ValidationError(_("No running contract found for the employee: "
        ...
        def _sum_salary_rule_category(localdict, category, amount):
            ...
            localdict['categories'].dict[category.code] =\
                (category.code in localdict['categories'].dict
                 and localdict['categories'].dict[category.code]
                 + amount or amount)
            return localdict
            ...
            def _sum(self, code, from_date, to_date=None):
                ...
                self.env.cr.execute(
                    """SELECT sum(number_of_days) as number_of_days,
                     sum(number_of_hours) as number_of_hours FROM
                      hr_payslip as hp, hr_payslip_worked_days as
                       pi WHERE hp.employee_id = %s AND
                        hp.state = 'done' AND hp.date_from >= %s AND
                         hp.date_to <= %s AND hp.id = pi.payslip_id AND
                          pi.code = %s""",
                    (self.employee_id, from_date, to_date, code))
            ...
            def sum(self, code, from_date, to_date=None):
                ...
                self.env.cr.execute(
                    """SELECT sum(case when hp.credit_note = False then
                     (pl.total) else (-pl.total) end) FROM
...
class HrPayslipLine(models.Model):
    """
        Represents each salary rule line in a payslip.

        Purpose:
        - Stores computed values for salary rules (amount, quantity, total).
        - Linked to employee, contract, and payslip.
    """
    _name = 'hr.payslip.line'
    _inherit = 'hr.salary.rule'
    _description = 'Payslip Line Item'
    ...
    def _compute_total(self):
        """Calculate total = quantity × amount × rate%."""
        for line in self:
    ...
    def create(self, vals_list):
        for values in vals_list:
            if 'employee_id' not in values or 'contract_id' not in values:
                ...
                values['contract_id'] =\
                    values.get('contract_id') or payslip.contract_id and payslip.contract_id.id
...
class HrPayslipWorkedDays(models.Model):
    """
        Represents worked days and hours linked to a payslip.

        Purpose:
        - Captures number of days and hours worked.
        - Used for salary computation (attendance, leaves, holidays).
    """
...
class HrPayslipInput(models.Model):
    """
        Additional inputs that impact payslip computation.

        Purpose:
        - Used for commissions, bonuses, or other custom inputs.
        - Linked to salary rules by code.
    """
    _name = 'hr.payslip.input'
    _description = 'Payslip Additional Input'
    _order = 'payslip_id, sequence'
...
class HrPayslipRun(models.Model):
    """
        ...
        Purpose:
        - Groups multiple payslips for a specific period.
        - Allows batch validation, closing, and refund generation.
    """
    ...
    credit_note = fields.Boolean(
        string='Credit Note',
        help="If its checked, indicates that all payslips"
             " generated from here are refund payslips."
    )
    ...
    def close_payslip_run(self):
        """Close the batch and restrict further changes."""
        return self.write({'state': 'close'})

    def done_payslip_run(self):
        """Validate all payslips in the batch and mark as done."""
        for line in self.slip_ids:
            line.action_payslip_done()
        return self.write({'state': 'done'})

    def unlink(self):
        """Prevent deletion of completed batches."""
        for rec in self:
            if rec.state == 'done':
                raise ValidationError(_(
                    'You Cannot Delete Done Payslips Batches'
                ))
